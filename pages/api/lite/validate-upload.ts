...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  form.parse(req, async (err, _fields, files) => {
    try {
      ...
      if (!isPDF && !isIMG)
        return res.status(400).json({ ok: false, msg: "Upload a PDF (preferred) or a clear image." });

      const sizeMB = f.size / (1024 * 1024);
      if (sizeMB < 0.05 || sizeMB > 15)
        return res.status(400).json({ ok: false, msg: "Invalid file size." });
      ...
      if (isIMG) {
        ...
        // Quick sharpness proxy: variance of greyscale pixels
        const raw = await img.greyscale().raw().toBuffer({ resolveWithObject: true });
        const data = raw.data as Uint8Array;
        const mean = data.reduce((a, b) => a + b, 0) / data.length;
        const variance = data.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / data.length;
        if (variance < 50)
          return res.status(400).json({ ok: false, msg: "Image looks blurry—try a clearer shot." });
      }
