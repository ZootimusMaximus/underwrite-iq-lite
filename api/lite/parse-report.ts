...
function extractFrom(text: string) {
  ...
  let largestInstallment = 0, largestUnsecuredInstallment = 0, seasonedLargestUnsecuredInstallment = 0;
  ...
  while ((loanMatch = LOAN_AMT_RE.exec(text)) !== null) {
    ...
    if (!looksSecured) {
      ...
      const d = od ? parseOpenedDate(od) : null;
      const seasoned = d ? monthsSince(d) >= CONFIG.seasoningMonths : false;
      if (seasoned) seasonedLargestUnsecuredInstallment = Math.max(seasonedLargestUnsecuredInstallment, val);
    }
  }
  ...
  return { score, util, inq: { ex, tu, eq }, negatives_count, negLines, avgAgeYears,
           highestCardLimit, seasonedHighestCardLimit, largestInstallment,
...
function gradeFromPoints(n:number):Grade{if(n>=11)return"A";if(n>=9)return"B";if(n>=7)return"C";return"D";}
...
export default function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  form.parse(req,async(err,fields,files)=>{
    try{
      if(err)return res.status(400).json({ok:false,msg:"Upload parsing failed"});
      const uploaded=files.file;
      const fileArr:Array<FormidableFile>=Array.isArray(uploaded)?uploaded as FormidableFile[]:uploaded?[uploaded as FormidableFile]:[];
      if(!fileArr.length)return res.status(400).json({ok:false,msg:"Please upload 1–3 bureau PDF reports."});

      // ---- FIXED BUSINESS INPUTS ----
      const businessNameRaw=fields.businessName;
      const businessAgeRaw=fields.businessAgeMonths;
      ...
      for(const f of fileArr){
        ...
        if(!/\.pdf$/.test(name))return res.status(400).json({ok:false,msg:"Only PDF credit reports supported."});
      ...
      let wEX=exData?CONFIG.weights.EX:0, wTU=tuData?CONFIG.weights.TU:0, wEQ=eqData?CONFIG.weights.EQ:0;
      ...
      const exInq=exData?.inq.ex??0, tuInq=tuData?.inq.tu??0, eqInq=eqData?.inq.eq??0;
      const inquiries={ex:exInq,tu:tuInq,eq:eqInq};
      const totalInquiries=exInq+tuInq+eqInq;
      const negatives_count=Math.max(exData?.negatives_count??0,tuData?.negatives_count??0,eqData?.negatives_count??0,fbData?.negatives_count??0);
      ...
      const fundable=(score!=null&&score>=gate.minScore)&&(util==null||util<=gate.maxUtil)&&negatives_count<=gate.maxNegatives&&totalInquiries<=gate.maxInquiriesTotal;

      // Pillars
      const negLines=(exData?.negLines||[]).concat(tuData?.negLines||[],eqData?.negLines||[],fbData?.negLines||[]);
      ...
      const PH=negatives_count===0?3:hasBK?1:(negLines.some(l=>l.includes("collection")||l.includes("charge-off"))?1:2);
      const UT=(util==null?(dataQualityConstrained=true,2):util<10?3:util<=30?2:util>50?1:2);
      const DA=(avgAgeYears==null?(dataQualityConstrained=true,openAccounts>3?2:1):avgAgeYears>=5&&openAccounts>=5?3:avgAgeYears>=3?2:1);
      const NR=(newAccounts12mo==null?(dataQualityConstrained=true,2):newAccounts12mo===0?3:newAccounts12mo<=2?2:1);
      ...
      const toGrade=(forCards:boolean):Grade=>{
        ...
        if(hardCap){const cIdx=order.indexOf(hardCap);const gIdx=order.indexOf(g);if(gIdx<cIdx)g=hardCap;}
        if(dataQualityConstrained)g=downgrade(g);
        return g;
      };

      let gradeCard=toGrade(true), gradeLoan=toGrade(false);
      ...
      return res.status(200).json({
        ok:true,
        outputs:{
          fundable,banner_estimate,personal_total_max,business_total_max,
          personal_card,personal_loan,
          meta:{score_estimate:score,utilization_pct:util,negatives_count,avg_age_years:avgAgeYears}
        }
      });
    }catch(e:any){
      console.error(e);
